    ...
    void ispis() {
        ...
        for (int i = 0; i < getBrojRedova(); i++) {
            for (int j = 0; j < getBrojKolona(); j++) {
                cout << "[" << _sjedista[i][j].getRed() << "-" << _sjedista[i][j].getKolona() << "]";
            }
            cout << endl;
        }
        cout << "-----------------------------" << endl;
...
public:
    ...
    Datum getDatumRodjenja() const {
        Datum temp;
        temp.unos(1, 1, 1970);
        return (_datumRodjenja == nullptr) ? temp : *_datumRodjenja;
    }
    bool getSpol() const { return (_spol == nullptr) ? true : *_spol; }     // po defaultu su svi muskog spola
...
public:
    ...
    void setDvorane(int maxBrojDvorana) {
        ...
        *_maxBrojDvorana = maxBrojDvorana;
        ...
        _dvorane = new Dvorana[maxBrojDvorana];
    }
    ...
    bool dodajDvoranu(Dvorana& d) {
        if (_trenutnoDvorana >= getMaxBrojDvorana())
            return false;
        _dvorane[_trenutnoDvorana].unos(d);
        _trenutnoDvorana++;
        return true;
    }
    //Z4.7
    bool dodajUposlenika(Uposlenik& u) {
        ...
        _uposlenici[_trenutnoUposlenika]->unos(u);
        _trenutnoUposlenika++;
        return true;
    }
    //Z4.8
    ...
    void ispis() {
        cout << "Naziv kina: " << getNaziv() << endl;
        cout << "Dvorane: " << endl;
        cout << "------------------------------------" << endl;
        for (int i = 0; i < getTrenutnoDvorana(); i++) {
            cout << "[" << i << "]: " << _dvorane[i].getNaziv() << endl;
        }
        cout << "------------------------------------\n" << endl;
        ...
        for (int i = 0; i < getTrenutnoUposlenika(); i++) {
            cout << "[" << i << "]: " << _uposlenici[i]->getIme() << " " << _uposlenici[i]->getPrezime() << endl;
        }
        cout << "------------------------------------\n" << endl;
    }

    void dealocirajDvorane() {
        for (int i = 0; i < getTrenutnoDvorana(); i++) {
...
void zadatak4() {
    ...
    u3.unos("Tom", "Cruise", datum3, 1, "vozac", "Brcko", "thomas_cruise@gmail.com");
