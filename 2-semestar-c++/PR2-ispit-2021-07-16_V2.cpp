...
public:
	...
	T1& operator[](int el2Vrijednost) {
		for (int i = 0; i < getTrenutno(); i++) {
			if (*_elementi2[i] == el2Vrijednost)
				return *_elementi1[i];
		}
	}
};
	...
	*/
	bool AddOcjena(int ocjena, Datum& datum) {
		...
		if (_ocjene.getTrenutno() > 0) {
			if (abs(datum.toInt() - _ocjene.getElement2(_ocjene.getTrenutno() - 1)->toInt()) < 3)
				return false;
		}
	...
	friend ostream& operator<< (ostream& COUT, const Tehnika& obj) {
		COUT << crt << "Naziv tehnike: " << obj.GetNaziv() << "\nOcjene:\n";
		...
		COUT << "Prosjecna ocjena za tehniku: " << obj.getProsjecnaOcjenaZaTehniku() << crt;
		return COUT;
	}
...
public:
	...
	friend ostream& operator<< (ostream& COUT, const Polaganje& obj) {
		COUT << crt << "Pojas: " << obj._pojas << endl;
	...
	float getProsjecnuOcjenuSvihTehnika() const {
		float suma = 0.0f;
		int brojac = 0;
...
bool ValidirajLozinku(string lozinka) {
	return regex_search(lozinka, regex("(?=.*.{7,})(?=.*[A-Z])(?=.*[a-z])(?=.*\\W)(?=.*\\d)"));
}
/*
...
public:
	...
	/*
	svi kandidati podrazumijevano imaju BIJELI pojas (za njega se ne dodaju tehnike) sve tehnike na nivou jednog pojasa (ZUTI, ZELENI ... ) se evidentiraju unutar istog objekta tipa Polaganje, tom prilikom onemoguciti:
	
	- dodavanje istih (moraju biti identicne vrijednosti svih atributa) tehnika na nivou jednog pojasa,
	- dodavanje tehnika za visi pojas ako prethodni pojas nema evidentirane najmanje 3 tehnike ili nema prosjecnu ocjenu svih tehnika vecu od 3.5 (onemoguciti dodavanje tehnike za NARANDZASTI ako ne postoji najmanje 3 tehnike za ZUTI pojas i njihov prosjek je veci od 3.5)
	...
	*/
	...
	bool AddTehniku(Pojas pojas, Tehnika& tehnika) {
		for (int i = 0; i < _polozeniPojasevi[pojas]->GetTehnike().size(); i++) {
			if (tehnika == *_polozeniPojasevi[pojas]->GetTehnike()[i])
				return false;
		}

		if (pojas != ZUTI) {
			if (_polozeniPojasevi[pojas]->GetTehnike().size() < 3 || _polozeniPojasevi[pojas]->getProsjecnuOcjenuSvihTehnika() < 3.5)
				return false;
		}

		_polozeniPojasevi[pojas]->GetTehnike().push_back(new Tehnika(tehnika));

		thread SlanjeEmaila(&KaratePolaznik::PosaljiEmail, this, tehnika.GetNaziv(), pojas, _polozeniPojasevi[pojas]->getProsjecnuOcjenuSvihTehnika(), getUkupniProsjek());
	...
	void PosaljiEmail(const char* nazivTehnike, Pojas pojas, float prosjekPojas, float ukupniProsjek) {
		mrPoliceman.lock();
		this_thread::sleep_for(chrono::seconds(3));
		cout << crt << "FROM: info@karate.ba\nTO: " << GetEmail();
		cout << "\nPostovani " << GetImePrezime() << ", evidentirana vam je tehnika " << nazivTehnike << " za " << pojas << " pojas.\n";
		cout << "Dosadasnji uspjeh (prosjek ocjena) na pojasu " << pojas << " iznosi  " << prosjekPojas << ", a ukupni uspjeh (prosjek ocjena) na svim pojasevima iznosi " << ukupniProsjek << ".\nPozdrav.\nKARATE Team." << crt;
		mrPoliceman.unlock();
	}
