...
class Kupac : public Osoba{
	...
	Rjecnik<float, ZadovoljstvoKupca>* _kupovine = nullptr;
...
public:
	...
	friend ostream& operator<< (ostream& COUT, const Kupac& obj) {
		COUT << crt << (Osoba&)(obj) << "Email adresa: " << obj._emailAdresa << " " << endl;
		COUT << "KUPOVINE -> " << crt;
   ...
   */
	...
	void Info() {  
		...
		for (int i = 0; i < _kupovine->getTrenutno(); i++) {
			cout << crt << "Iznos racuna: " << _kupovine->getElement1(i) << ", zadovoljstvo kupca: " << _kupovine->getElement2(i).GetOcjena() << endl;
			for (int j = 0; j < _kupovine->getElement2(i).GetKomentareKarakteristika().getTrenutno(); j++) {
				cout << "\t" << _kupovine->getElement2(i).GetKomentareKarakteristika().getElement1(j) << " - " << _kupovine->getElement2(i).GetKomentareKarakteristika().getElement2(j) << endl;
			}
		}
		cout << "BODOVI -> ";
	...
	void PosaljiEmail(int bodovi, int ukupnoBodovi) {
		mrPoliceman.lock();
		//this_thread::sleep_for(chrono::seconds(2));
		cout << crt << "To: denis@fit.ba;\nSubject: Ostvareni bodovi\n\nPostovani,\n\n";
		cout << "Prilikom posljednje kupovine ste ostvarili " << bodovi << " bodova, tako da trenutno vas ukupan broj bodova iznosi " << ukupnoBodovi << ".\n\n";
  ...
  */
  ...
  /*metoda SacuvajBodove treba sve clanove vector-a _bodovi upisati u fajl(ignorisuci ranije dodate/postojece vrijednosti u fajlu) pod nazivom emailKorisnika.txt npr. denis@fit.ba.txt.
	...
	void SacuvajBodove() {
		string naziv = GetEmail();
		naziv += ".txt";
		ofstream upisUFajl(naziv);
	...
	void UcitajBodove() {
		string naziv = GetEmail();
		naziv += ".txt";
		ifstream ispisIzFajla(naziv);

		string linija;

		while (getline(ispisIzFajla, linija)) {
			_bodovi.push_back(stoi(linija));
		}
	...
	/*Ocekivani ispis:
		...
		-------------------------------------------
	*/
	Rjecnik<Karakteristike, const char*> GetKupovineByKomentar(string komentar) {
		Rjecnik<Karakteristike, const char*> temp;
		for (int i = 0; i < _kupovine->getTrenutno(); i++) {
			for (int j = 0; j < _kupovine->getElement2(i).GetKomentareKarakteristika().getTrenutno(); j++) {
				if (regex_search(_kupovine->getElement2(i).GetKomentareKarakteristika().getElement2(j), regex(komentar)))
					temp.AddElement(_kupovine->getElement2(i).GetKomentareKarakteristika().getElement1(j), GetNizKaraktera(_kupovine->getElement2(i).GetKomentareKarakteristika().getElement2(j)));
			}
		}
		return temp;
	}
