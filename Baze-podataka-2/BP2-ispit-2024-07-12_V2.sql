...
SELECT
	*
FROM Proizvodi AS P
WHERE P.NazivProizvoda LIKE '%A%' AND P.NazivKategorije LIKE '%E'

--b) (4 boda) Kreirati proceduru sp_update_proizvodi kojom ce se izmjeniti podaci o proizvodima u tabeli. Korisnici mogu poslati jedan ili vise parametara te voditi raucna da ne dodje do gubitka podataka.(Nova baza)
...
RETURN
	SELECT
		N.NarudzbaID,
		SUM(SN.Cijena * SN.Kolicina) AS [Ukupna vrijednost bez popusta]
	FROM Narudzbe AS N
...
FROM
	...
	SELECT TOP 5
		...
		P.LastName,
		SOH.SalesOrderID,
		CAST(SUM(SOD.UnitPrice * SOD.OrderQty * (1 - SOD.UnitPriceDiscount)) AS DECIMAL(18, 2)) AS Ukupno,
		COUNT(DISTINCT SOD.ProductID) AS [Razliciti proizvodi/razlicite kategorije]
	FROM Sales.SalesOrderHeader AS SOH
...
FROM
	...
	SELECT TOP 5
		...
		SOH.SalesOrderID,
		CAST(SUM(SOD.UnitPrice * SOD.OrderQty * (1 - SOD.UnitPriceDiscount)) AS DECIMAL(18, 2)) AS Ukupno,
		COUNT(DISTINCT Prod.ProductSubcategoryID) AS [Razliciti proizvodi/razlicite kategorije]
	FROM Sales.SalesOrderHeader AS SOH
		INNER JOIN Sales.SalesOrderDetail AS SOD ON SOH.SalesOrderID = SOD.SalesOrderID
		INNER JOIN Sales.Customer AS C ON SOH.CustomerID = C.CustomerID
		INNER JOIN Person.Person AS P ON C.PersonID = P.BusinessEntityID
